...
# source ("utility.R")
#Rscript protein_single_feature_model_cross_validation.R  svm  10 5
  ...
  for(ti in 1:testNumber)
  {
    ...
    if(model=="xgboost"){
      ...
      for(i in 1:cvNumber){
        ...
        # xgb.save(xgb.valid.fit,sprintf("model/%s/%s_%s_cv_%s_%s_%s.h5",model,Type,model,fileTileVecs[fi],i,ti))
    ...
    if(model=="lightgbm"){
      ...
      for(i in 1:cvNumber){
        ...
        params <- list(
          ...
          max_depth =  as.integer(Args$maxDepth)[which(Args==fileTitle,arr.ind=T)[[1]]],
        ...
        # lgb.save(lgb_mod,sprintf("model/%s/%s_%s_cv_%s_%s_%s.model",model,Type,model,fileTileVecs[fi],i,ti))
...
if(dir.exists("result")){
  ...
  write.csv(matt,file=sprintf("result/%s_single_feature_%s_cv_perc.csv",Type,model),row.names = F)
  
  colnames(mat) <- c("Model","Encoding","PRE","SN","SP","F-value","ACC","MCC")
  write.csv(mat,file=sprintf("result/%s_single_feature_%s_cv_perf.csv",Type,model),row.names = F)
  colnames(mattt)<-c(feature_name,"LabelList","foldList")
  write.csv(mattt,file=sprintf("result/%s_single_feature_%s_cv_label.csv",Type,model),row.names = F)
}else{
  dir.create("result")
  write.csv(matt,file=sprintf("result/%s_single_feature_%s_cv_perc.csv",Type,model),row.names = F)
...
inputFile = vector()
  ...
  for (i in 1:length(fileTileVecs)){
    ...
    for(fnum in 1:times){
      ...
      inputFile = c(inputFile,sprintf("feature_train/%s/%s_train_%s.csv",fnum,Type,fileTileVecs[i]))

      tuneFile = c(tuneFile,sprintf("table/%s_tune_%s_%s.csv",Type,model,times))
    }
