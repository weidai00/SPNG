...
getTrainingModel <- function(Type,method, trainData,  fileTile, times, ArgsFile) {
  ...
  if(method=="svm"){
    ...
    trainModel <- naiveBayes(Class~.,data=trainData,type="raw",laplace=0)
    saveRDS(trainModel,sprintf("model/%s_%s_%s_%s.rds",Type,fileTile,method,times))
   }
...
for(i in 1:times){
  ...
  for(fi in 1:length(fileVecs)) {
   ...
   file = sprintf("feature_train/%s/%s_train_%s.csv",i,Type,fileVecs[fi])
    
    # if(fileTileVecs[fi] == "S-FPSSM" || fileTileVecs[fi] == "SOCN" || fileTileVecs[fi] == "MORAN"){
    #   next
    # }
    print(fileTileVecs[fi])
    getTrainingModel(Type=Type,method = "svm", trainData=file,  fileTile=fileTileVecs[fi], times=i, ArgsFile=sprintf("table/%s_tune_svm_%s.csv",Type,i))
    ...
    getTrainingModel(Type=Type,method = "rf", trainData=file,  fileTile=fileTileVecs[fi],times = i,ArgsFile = sprintf("table/%s_tune_rf_%s.csv",Type,i))
    getTrainingModel(Type=Type,method = "lightgbm", trainData=file,  fileTile=fileTileVecs[fi], times=i, ArgsFile=sprintf("table/%s_tune_lightgbm_%s.csv",Type,i))
    getTrainingModel(Type=Type,method = "xgboost", trainData=file,  fileTile=fileTileVecs[fi],time=i,ArgsFile =sprintf("table/%s_tune_xgboost_%s.csv",Type,i) )
